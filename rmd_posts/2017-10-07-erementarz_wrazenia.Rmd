...
read.csv("./rmd_posts/erementarz_zapisy.csv") %>% 
  mutate(dzien = as.Date(data),
         plec = factor(plec, labels = c("Kobieta", "Mężczyzna"))) %>% 
  group_by(dzien, plec, typ) %>% 
  summarise(total = length(plec)) %>% 
  ungroup() %>% 
  arrange(plec, typ, dzien) %>% 
  group_by(plec, typ) %>% 
  mutate(ctotal = cumsum(total)) %>% 
  ggplot(aes(x = dzien, y = ctotal, color = plec, linetype = typ)) +
  geom_line() +
  #geom_text(stat = "count", aes(label = ..count..), position = "dodge") +
  scale_x_date("Dzień", date_breaks = "2 days") +
  scale_y_continuous("Zapisani uczestnicy") + 
  scale_color_manual("Płeć", values = c("blue", "red")) +
  scale_linetype("Typ rejestracji") +
  theme_bw() +
